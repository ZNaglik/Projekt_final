    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->actionZa_aduj->setEnabled(true);
    ui->actionPoka_dane->setEnabled(false);
    ui->actionWyczy->setEnabled(false);
    ui->actionWykres_s_upkowy_2->setEnabled(false);
    ui->actionWykres_liniowy->setEnabled(false);
    ui->actionWykres_ko_owy->setEnabled(false);
   ...
   // tableWidget->setGeometry(QApplication::desktop()->screenGeometry());
    ...
    setWindowTitle(tr("Wykresy - ekran glowny"));
...
void MainWindow::checkForCharts()
{
    if(circleChartAvl)
    {
        ...
        ui->actionWykres_ko_owy->setEnabled(true);
    }
    else
    {
        ui->circleChart_Btn->setEnabled(false);
    ...
    if(barChartAvl)
    {
        ...
        ui->actionWykres_s_upkowy_2->setEnabled(true);
    }
    else
    {
        ui->barChart_Btn->setEnabled(false);
    ...
    {             
        ...
        ui->actionWykres_liniowy->setEnabled(true);
    }
    else
    {
        ui->lineChart_Btn->setEnabled(false);
...
void MainWindow::on_actionZa_aduj_triggered()
{
    loadData();
}
...
void MainWindow::loadData()
{
    ...
    if(isLoaded) {
        ...
        ui->actionZa_aduj->setEnabled(false);
        ui->actionPoka_dane->setEnabled(true);
        ui->actionWyczy->setEnabled(true);
        ui->actionWykres_s_upkowy_2->setEnabled(false);
        ui->actionWykres_liniowy->setEnabled(false);
        ui->actionWykres_ko_owy->setEnabled(false);
        ...
        while((col <= ui->tableWidget->columnCount()) && (stopLoading == false))
        {
            ...
            while((row <= ui->tableWidget->rowCount()) && (row <= presentRows))
            {
                   ...
                   for (int j = 0; j < tempStr.size(); j++)
                   {
                       if(tempStr[j] == ',')
                       {
                           if(commaPresent == false)
                           {
                                ...
                                //w PL uzywamy przecinkow, a w QT kropek
                                tempStr[j] = '.';
                           }
                           else
                           {
                       ...
                       if(tempStr[j].isDigit() || tempStr[j] == ".")
                       {
                           //nic nie rob
                       }
                       else
                ...
                if(tempStr == "")
                {
                    //wartosc liczbowa domyslna
                    dataVector.insert(pos, "0");
                }
                else {
                    dataVector.insert(pos, tempStr);
                    commaPresent = false;
                }
                pos++;
                row++;
